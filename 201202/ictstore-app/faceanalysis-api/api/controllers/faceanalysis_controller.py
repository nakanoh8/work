...
def auth():
    ...
    # 認証画像を保存
    faceanalysis.save_authimage(data["img"], config["auth_images_dir"], result_json)
    # レスポンス整形
...
def facedetection():
    data = json.loads(request.data.decode('utf-8'))
    result_json = faceanalysis.detect_face(data["img"])
...
def boundingbox():
    ...
    rgb_img = faceanalysis.load_rgbimg(data["img"])
...
def saveimage():
    # 登録画像(利用開始情報)を保存
    save_result = faceanalysis.save_registimage(data["img"], config["regist_images_dir"])
